	...
	<thead>
		<tr>
			<td valign="top" colspan="5">
				<select id="searchListValue" name="searchListValue" onchange="searchListValue()">
			...
			<td colspan="5">
				<div class="pagination">
					<c:if test="${paging.startBlock != 1}"> <%-- [이전으로] 처리 --%>
						<a href="/boardSearch?currentPage=${paging.startBlock - 1 }&countPerPage=${paging.countPerPage}&conSelect=${searchVO.conSelect}&searchData=${searchVO.searchData}">&lt;&lt;이전</a>
					</c:if>
					<c:forEach var="k" begin="${paging.startBlock }" end="${paging.endBlock }">
						...
						<c:if test="${paging.currentPage != k }">
							<a href="/boardSearch?currentPage=${k }&countPerPage=${paging.countPerPage}&conSelect=${searchVO.conSelect}&searchData=${searchVO.searchData} ">${k }</a>
						</c:if>
					</c:forEach>
					<c:if test="${paging.endBlock < paging.totalPageCount }"> <%-- [다음으로] 처리 --%>
						<a href="/boardSearch?currentPage=${paging.endBlock + 1 }&countPerPage=${paging.countPerPage}&conSelect=${searchVO.conSelect}&searchData=${searchVO.searchData} ">다음&gt;&gt;</a>
					</c:if>
				</div>
			</td>
		</tr>
				...
				<form action="/boardSearch">
					<div class="subWrite">
						<select name="conSelect" id="conSelect">
							<option value="tc" <c:if test="${searchVO.conSelect.equals('tc') }">selected</c:if>>제목+내용</option>
							<option value="t"  <c:if test="${searchVO.conSelect.equals('t') }">selected</c:if>>제목만</option>
							<option value="w"  <c:if test="${searchVO.conSelect.equals('w') }">selected</c:if>>글작성자</option>
						</select>
						
						<input type="text" placeholder="검색어를 입력해주세요" name="searchData" id="searchData" value="${searchVO.searchData }" required>
